...
save_model <- function(modelname) {
    ...
    saveRDS(efficacy_mat, file = here::here("outputs", modelname, "_wane.rds"))
}
...
fit_stan_exp <- readRDS(here::here("outputs", name, "fits_exp.rda"))
fit_stan_er2 <- readRDS(here::here("outputs", name, "fits_er2.rda"))
fit_stan_er3 <- readRDS(here::here("outputs", name, "fits_er3.rda"))
fit_stan_old <- readRDS(here::here("outputs", name, "fits_old.rda"))

if (name == "nmab_nobound") {
  ymax <- 0.0065
} else if (name == "mat_nobound") {
  ymax <- 0.00035
}

fit_stan_exp %>% 
  as_draws_df %>% 
  spread_draws(incidence_pla[t], time_p[t]) %>%
  ggplot() + 
  stat_lineribbon(aes(time_p, incidence_pla), .width = 0.95, fill = "blue", alpha = 0.5) + 
  coord_cartesian(ylim = c(0, ymax)) +
  ggtitle("New") +
  theme_bw() + labs(x = "Time post-vaccination (days)", y = "Incidence (placebo group)") -> p1
...
fit_stan_old %>% 
  ...
  coord_cartesian(ylim = c(0, ymax)) +
  ggtitle("Old") +
  theme_bw() + labs(x = "Time post-vaccination (days)", y = "Incidence (placebo group)") -> p1_old

g1 <- arrangeGrob(p1, p1_old, ncol = 2)
ggsave(file = paste0("outputs/", name, "/comp_incidence_pla.pdf"), g1)

fit_stan_exp %>% 
  as_draws_df %>% 
  spread_draws(waning[t]) %>% 
  ggplot() + 
  stat_lineribbon(aes(t, waning), .width = 0.95, fill = "red", alpha = 0.5) + theme_bw() +
  coord_cartesian(ylim = c(0, 1)) +
  ggtitle("New") +
  labs(x = "Time post-vaccination (days)", y = "Estimated efficacy") -> p2

fit_stan_old %>% 
  as_draws_df %>% 
  spread_draws(waning_exp[t]) %>% 
  ggplot() + 
  stat_lineribbon(aes(t, waning_exp), .width = 0.95, fill = "red", alpha = 0.5) + theme_bw() +
  coord_cartesian(ylim = c(0, 1)) +
  ggtitle("Old") +
  labs(x = "Time post-vaccination (days)", y = "Estimated efficacy") -> p2_old

g2 <- arrangeGrob(p2, p2_old, ncol = 2)
ggsave(file = paste0("outputs/", name, "/comp_exp.pdf"), g2)
...
fit_stan_er2 %>% 
  ...
  coord_cartesian(ylim = c(0, 1)) +
  ggtitle("New") +
  labs(x = "Time post-vaccination (days)", y = "Estimated efficacy") -> p3

fit_stan_old %>% 
  as_draws_df %>% 
  spread_draws(waning_er2[t]) %>% 
  ggplot() + 
  stat_lineribbon(aes(t, waning_er2), .width = 0.95, fill = "red", alpha = 0.5) + theme_bw() +
  coord_cartesian(ylim = c(0, 1)) +
  ggtitle("Old") +
  labs(x = "Time post-vaccination (days)", y = "Estimated efficacy") -> p3_old

g3 <- arrangeGrob(p3, p3_old, ncol = 2)
ggsave(file = paste0("outputs/", name, "/comp_er2.pdf"), g3)
...
fit_stan_er3 %>% 
  ...
  coord_cartesian(ylim = c(0, 1)) +
  ggtitle("New") +
  labs(x = "Time post-vaccination (days)", y = "Estimated efficacy") -> p4

fit_stan_old %>% 
  as_draws_df %>% 
  spread_draws(waning_er3[t]) %>% 
  ggplot() + 
  stat_lineribbon(aes(t, waning_er3), .width = 0.95, fill = "red", alpha = 0.5) + theme_bw() +
  coord_cartesian(ylim = c(0, 1)) +
  ggtitle("Old") +
  labs(x = "Time post-vaccination (days)", y = "Estimated efficacy") -> p4_old

g4 <- arrangeGrob(p4, p4_old, ncol = 2)
ggsave(file = paste0("outputs/", name, "/comp_er3.pdf"), g4)
