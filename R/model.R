...
fitted_functions_none <- function(data_vac, name) {
    ...
    T <- max(data_vac$placebo$t)
    mod <- cmdstan_model(here::here("src", "eff_est_none.stan"), compile = TRUE)

    fit_stan_exp <- mod$sample(
        data = c(model_id = 1, data_list), 
     #   seed = 123, 
        chains = 4, 
        parallel_chains = 4,
        refresh = 500 # print update every 500 iters
