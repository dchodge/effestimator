...
parameters{
    real<lower = 0, upper = 1> wane_a; //??wane a rate for exponential waning
    real<lower = 0, upper = lower_bound> wane_b; //??wane b rate for exponential waning
...
model {
    ...
    if (model_id == 1) {
      n_vac ~ poisson(persons_vac .* exp(inci_gp) .* (1 - exp_wane(wane_a, 1.0/wane_b, t_vac)));
    } else if (model_id == 2) {
      n_vac ~ poisson(persons_vac .* exp(inci_gp) .* (1 - erlangk_wane(wane_a, 1.0/wane_b, t_vac, 2)));
    } else if (model_id == 3) {
      n_vac ~ poisson(persons_vac .* exp(inci_gp) .* (1 - erlangk_wane(wane_a, 1.0/wane_b, t_vac, 3)));
    }
...
generated quantities {
    ...
    if (model_id == 1) {
      waning = exp_wane(wane_a, 1.0/wane_b, time_points);
    } else if (model_id == 2) {
      waning = erlangk_wane(wane_a, 1.0/wane_b, time_points, 2);
    } else if (model_id == 3) {
      waning = erlangk_wane(wane_a, 1.0/wane_b, time_points, 3);
    }

    // Waning efficacy at time points in data
    vector[N_pla] waning_inc;

    vector[N_pla] time_p;
    if (model_id == 1) {
      waning_inc = exp_wane(wane_a, 1.0/wane_b, t_vac);
    } else if (model_id == 2) {
      waning_inc = erlangk_wane(wane_a, 1.0/wane_b, t_vac, 2);
    } else if (model_id == 3) {
      waning_inc = erlangk_wane(wane_a, 1.0/wane_b, t_vac, 3);
    }
    ...
    for(i in 1:N_pla) {
        ...
        incidence_vac[i] = exp(inci_gp[i]) * (1 - waning_inc[i]);
    }
}
